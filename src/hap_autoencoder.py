...
weights_path = '../res/hands_autoencoder_weights_d_{}.pt'.format(d)
save_encoded_dir = '../data/HaP/d_{}/'.format(d)
save_encoded_path = save_encoded_dir + 'hands_{}_images_encoded_d_{}.pt'
make_dirs([save_encoded_dir])

wandb_project_name = 'HaP Autoencoder'
...
if __name__ == '__main__':
    ...
    make_dirs(['../res/', save_encoded_dir])

    img_dir = '../data/HaP/hands_kaggle/Hands/Hands/'
    csv_dir = '../data/HaP/'

    custom_transform = transforms.Compose([transforms.Resize((64, 64)),
                                           transforms.ToTensor(),
                                           transforms.Normalize((0.5,0.5, 0.5), (0.5,0.5,0.5))])  # commonly used normlization (see: https://discuss.pytorch.org/t/understanding-transform-normalize/21730/14 )

    train_dataset = HandsDataset(csv_path=csv_dir + 'hands_kaggle-train.csv',
                                  img_dir=img_dir,
                                  transform=custom_transform)

    val_dataset = HandsDataset(csv_path=csv_dir + 'hands_kaggle-val.csv',
                                  img_dir=img_dir,
                                  transform=custom_transform)

    test_dataset = HandsDataset(csv_path=csv_dir + 'hands_kaggle-test.csv',
                                 img_dir=img_dir,
                                 transform=custom_transform)
    ...
    for data_loader, data_loader_name in [(train_dataloader, 'train'), (val_dataloader, 'val'),
        ...
        torch.save(all_encoded_images,
                   save_encoded_path.format(data_loader_name, d))
